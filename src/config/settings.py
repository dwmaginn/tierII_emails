...
class TierIISettings(BaseSettings):
    ...
    # Test Configuration
    test_recipient_email: Optional[str] = Field(None, alias="TIERII_TEST_RECIPIENT_EMAIL")
    test_fallback_first_name: str = Field("Friend", alias="TIERII_TEST_FALLBACK_FIRST_NAME")
    ...
    def validate_batch_size(cls, v):
        ...
        if v > 1000:
            logger.warning(f"Large batch size detected: {v}. Consider smaller batches for better performance.")
        return v
    ...
    def validate_delay_minutes(cls, v):
        ...
        if v > 60:
            logger.warning(f"Long delay detected: {v} minutes. This may slow down campaigns significantly.")
        return v
...
def load_settings(test_mode: bool = False) -> TierIISettings:
    ...
    try:
        ...
        if test_mode and not settings.test_recipient_email:
            logger.error("TIERII_TEST_RECIPIENT_EMAIL is required when running in test mode")
            if is_testing:
                raise SystemExit(1)
            else:
                sys.exit(1)
        ...
        logger.info(f"Campaign Settings: Batch={settings.campaign_batch_size}, Delay={settings.campaign_delay_minutes}min")
        
        if settings.email_template_path:
            logger.info(f"Email Template: {settings.email_template_path}")
        else:
            logger.info("Email Template: Using inline HTML (no template file specified)")
        ...
        if test_mode:
            logger.error("  TIERII_TEST_RECIPIENT_EMAIL - Test recipient (required in test mode)")
...
def _get_legacy_settings():
    ...
    if _legacy_settings is None:
        try:
            ...
            logger.warning("Using legacy configuration constants. Consider migrating to load_settings() function.")
        except SystemExit:
...
_LEGACY_CONSTANT_NAMES = {
    'SENDER_EMAIL', 'SMTP_SERVER', 'SMTP_PORT', 'TENANT_ID', 
    'CLIENT_ID', 'CLIENT_SECRET', 'BATCH_SIZE', 'DELAY_MINUTES'
}
