...
class TestOAuthTokenManager:
    ...
    @pytest.mark.oauth
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_success(self, mock_post, token_manager, mock_successful_response, freeze_time):
        ...
        # Verify token expiry is set correctly (3600 - 300 = 3300 seconds from now)
        from datetime import datetime, timedelta
        ...
        assert 'https://login.microsoftonline.com/test-tenant-id/oauth2/v2.0/token' in call_args[0][0]
    ...
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_uses_cached_token(self, mock_post, token_manager, freeze_time):
        """Test that cached token is used when still valid."""
        from datetime import datetime, timedelta
        
        # Set up existing valid token
        token_manager.access_token = 'cached_token_12345'
        token_manager.token_expiry = datetime.now() + timedelta(minutes=30)  # Valid for 30 more minutes
    ...
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_refreshes_expired_token(self, mock_post, token_manager, mock_successful_response, freeze_time):
        """Test that expired token is refreshed."""
        from datetime import datetime, timedelta
        
        # Set up expired token
        token_manager.access_token = 'expired_token'
        token_manager.token_expiry = freeze_time - timedelta(minutes=5)  # Expired 5 minutes ago
    ...
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_http_error(self, mock_post, token_manager, mock_error_response):
    ...
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_custom_expires_in(self, mock_post, token_manager, freeze_time):
        ...
        expected_expiry = freeze_time + timedelta(seconds=6900)
        assert token_manager.token_expiry == expected_expiry

    @pytest.mark.oauth
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_missing_expires_in(self, mock_post, token_manager, freeze_time):
        ...
        from datetime import timedelta
        expected_expiry = freeze_time + timedelta(seconds=3300)
        assert token_manager.token_expiry == expected_expiry
    ...
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_get_access_token_missing_access_token_in_response(self, mock_post, token_manager):
    ...
    @pytest.mark.unit
    @patch('src.email_campaign.requests.post')
    def test_multiple_token_requests(self, mock_post, token_manager, mock_successful_response):
